...
# ----------------------------------------
def simulate_multiple_walks(p_dict, horizon=252, n_paths=1000, seed=None):
    """
    Simulate multiple discrete random walks using a single random seed.

    Parameters:
    - p_dict: dictionary with probabilities for each step, where keys are -1, 0, and +1
    - horizon: number of time steps to simulate for each path
    - n_paths: total number of independent paths to generate
    - seed: base random seed for reproducibility (same seed → identical results)

    Returns:
    - walks: np.ndarray of shape (n_paths, horizon), containing simulated step values
             for each path.
    """
    
    steps_values = np.array(list(p_dict.keys())) # [-1, 0, 1]
    weights = np.array([p_dict[-1], p_dict[0], p_dict[1]])
    
    walks = np.zeros((n_paths, horizon))
    ...
    return walks # 1000 simulated scaled Wt walks 

# ----------------------------------------
# STEP 3: Forecast from multiple walks
# ----------------------------------------
def forecast_from_multiple_walks(train_df, walks, k, mu, log_rate_col='log_rate'):
    """
    ...
    """
    X_o = float(train_df[log_rate_col].iloc[-1])
    n_paths, horizon = walks.shape

    # unscale the walks
    walks_unscaled = walks / k  # shape (n_paths, horizon)
    sum_walks = np.cumsum(walks_unscaled, axis=1)  # shape (n_paths, horizon)

    time_steps = np.arange(1, horizon+1,dtype=float)
    
    # predicted log-rates = X_o + mu * time_steps + sum(walks)
    predicted_log_rates = X_o + mu * time_steps + sum_walks  # shape (n_paths, horizon)
...
def plot_multiple_paths_vs_history(train_df, df_paths, df_test=None, rate_col='rate', n_show=30):
    ...
    plt.title(f'Discrete Random Walk — {steps}-day simulations ({df_paths.shape[0]} Paths)')
    plt.xlabel('Date')
